...
const invalidClientType uint8 = 255
...
func mustStartMiniRedis(t *testing.T) *goredis.Client {
	t.Helper()
...
func TestClientAdapter(t *testing.T) {
	...
	simpleType := goredis.NewClient(&redis.Options{Addr: "127.0.0.1:0"}).Type
	clusterType := goredis.NewClusterClient(&redis.ClusterOptions{Addrs: []string{"127.0.0.1:0"}}).Type
	testType := mustStartMiniRedis(t).Type
...
func TestClient_Ping(t *testing.T) {
	...
	t.Run("cluster type backed by a non-cluster client fails to cast", func(t *testing.T) {
		...
		client, err := goredis.ClientAdapter(uc, clusterType)
		require.NoError(t, err)
